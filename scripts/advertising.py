...
class ReportsClient:
    ...
    def post_report(self, body):
        ...
        try:
            ...
            print(f"post_report reportId={resp.payload['reportId']}")
    ...
    def get_report(self, report_id: str, retry_num=10):
        """
        ...
        try:
            ...
            if not resp.payload['url'] and retry_num > 0:
                retry_num -= 1
                print(f"get_report retry {resp.payload}")
                time.sleep(60)
                self.get_report(report_id=report_id, retry_num=retry_num)
            print(f"get_report [{resp.payload['url']}]")
    ...
    def get_display_report(self):
        ...
        url = self.get_report(report_id=report_id)
        if not url:
            print(f"{name} not url")
            return
        
        time.sleep(5)
        self.download_report(url=url, filename="display")
    ...
    def get_product_report(self):
        ...
        time.sleep(5)
        self.download_report(url=url, filename="product")
    ...
    def get_brands_report(self):
        ...
        time.sleep(5)
        self.download_report(url=url, filename="brands")
